...
function scanLink(link) {
    ...
    chrome.tabs.create({url: chrome.extension.getURL(SCAN_PAGE)}, function(tab) {
        chrome.tabs.onUpdated.addListener(function onUpdated(updatedTabId, updatedTabInfo) {
            if (tab.id == updatedTabId && updatedTabInfo.status === "complete") {
                chrome.tabs.sendMessage(tab.id, {method: "scanLink", url: link});
            }
        });
    });
}
